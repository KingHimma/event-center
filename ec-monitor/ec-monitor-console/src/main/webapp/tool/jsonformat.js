...
function Process(){
    ...
    var json = $id("respContent").value;
    var html = "";
    try{
        if(json == "") json = "\"\"";
        var obj = eval("["+json+"]");
        html = ProcessObject(obj[0], 0, false, false, false);
        $id("respContentDiv").innerHTML = "<PRE class='CodeContainer'>"+html+"</PRE>";
    }catch(e){
        alert("JSON数据格式不正确:\n"+e.message);
        $id("respContentDiv").innerHTML = "";
    }
}
...
function CollapseAllClicked(){
    ...
    TraverseChildren($id("respContentDiv"), function(element){
...
function CollapseLevel(level){
    ...
    TraverseChildren($id("respContentDiv"), function(element, depth){
...
function EnsureIsPopulated(){
    ...
    if(!$id("respContentDiv").innerHTML && !!$id("respContent").value) Process();
...
function SelectAllClicked(){
    ...
    if(!!range.selectNode)
        ...
        range.selectNode($id("respContentDiv"));

    else if(range.moveToElementText)

        range.moveToElementText($id("respContentDiv"));



    if(!!range.select)

        range.select($id("respContentDiv"));
